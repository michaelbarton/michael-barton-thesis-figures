...
plot <- xyplot(
  ...
  panel=function(x,y,subscripts){
    ...
    spearman = cor.test(panel_data$weight,panel_data$count,method="spearman")
    panel.text(95,135000, paste("R = ",round(spearman$estimate,digits=3)))
    panel.text(95,125000, paste("p = ",round(spearman$p.value,digits=4)))
  }
)
